...
def get_pool_config() -> PoolManagerConfig:
    ...
    环境变量:
        USE_ENHANCED_POOL: 是否使用增强的连接池管理(默认True)
        POOL_STATE_LOCK_TIMEOUT: 状态锁超时时间(默认10.0秒)
        HEALTH_CHECK_TIMEOUT: 健康检查超时时间(默认5.0秒)
        CLEANUP_INTERVAL: 后台清理任务间隔(默认300秒)
        MAX_CLEANUP_TIME: 单次清理最大时间(默认30秒)
        MAX_CLEANUP_QUEUE_SIZE: 清理队列最大大小(默认100)
        LEAK_WARNING_THRESHOLD: 泄漏警告阈值(默认80%)
        LEAK_CRITICAL_THRESHOLD: 泄漏严重阈值(默认90%)
    ...
    return PoolManagerConfig(
        ...
        health_check_sql=os.getenv("HEALTH_CHECK_SQL", "SELECT 1"),
        cleanup_interval=int(os.getenv("CLEANUP_INTERVAL", "300")),
        max_cleanup_time=int(os.getenv("MAX_CLEANUP_TIME", "30")),
        max_cleanup_queue_size=int(os.getenv("MAX_CLEANUP_QUEUE_SIZE", "100")),
        leak_warning_threshold=int(os.getenv("LEAK_WARNING_THRESHOLD", "80")),
        leak_critical_threshold=int(os.getenv("LEAK_CRITICAL_THRESHOLD", "90")),
...
def is_enhanced_pool_enabled() -> bool:
    ...
    Returns:
        bool: 是否启用
    """
    return os.getenv("USE_ENHANCED_POOL", "true").lower() in ("true", "1", "yes")
