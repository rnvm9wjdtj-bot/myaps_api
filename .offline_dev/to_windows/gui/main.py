...
class OfflineMigrationTool:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("MyAPS API 离线迁移工具")
    ...
    def setup_ui(self):
        ...
        self.log("欢迎使用 MyAPS API 离线迁移工具！")
    ...
    def setup_download_tab(self):
        ...
        self.combo_platform = ttk.Combobox(form_frame, values=["win_amd64", "win32"], state="readonly", width=47)
        self.combo_platform.set("win_amd64")
        ...
        self.entry_output_dir.insert(0, str(self.project_root / ".offline_dev" / "packages"))
    ...
    def setup_install_tab(self):
        ...
        self.entry_packages_dir.insert(0, str(self.project_root / ".offline_dev" / "packages"))
        ...
        tk.Label(frame, text="说明：请确保已将 .offline_dev/packages/ 目录复制到内网。", fg="gray").pack(pady=10)
    ...
    def _install_worker(self):
        ...
        try:
            ...
            if not packages_dir.exists():
                ...
                self.log("请确认已将 .offline_dev/packages/ 目录复制到内网！", "error")
                return
    ...
    def setup_service_tab(self):
        ...
        tk.Label(frame, text="服务管理", font=("Arial", 14, "bold")).pack(pady=(0, 20))
        ...
        tk.Label(form_frame, text="虚拟环境名称:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.entry_service_venv_name = tk.Entry(form_frame, width=50)
        self.entry_service_venv_name.insert(0, "venv")
        self.entry_service_venv_name.grid(row=1, column=1, sticky=tk.EW, pady=5)

        tk.Label(form_frame, text="监听端口:").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.entry_service_port = tk.Entry(form_frame, width=50)
        self.entry_service_port.insert(0, "8000")
        self.entry_service_port.grid(row=2, column=1, sticky=tk.EW, pady=5)

        tk.Label(form_frame, text="监听地址:").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.entry_service_host = tk.Entry(form_frame, width=50)
        self.entry_service_host.insert(0, "0.0.0.0")
        self.entry_service_host.grid(row=3, column=1, sticky=tk.EW, pady=5)

        form_frame.columnconfigure(1, weight=1)

        btn_frame = tk.Frame(frame)
        btn_frame.pack(pady=20)
        self.btn_start = tk.Button(btn_frame, text="启动服务", command=self.start_service, bg="#4CAF50", fg="white", font=("Arial", 12, "bold"), padx=30, pady=10)
        self.btn_start.pack(side=tk.LEFT, padx=10)
        self.btn_stop = tk.Button(btn_frame, text="停止服务", command=self.stop_service, bg="#F44336", fg="white", font=("Arial", 12, "bold"), padx=30, pady=10, state=tk.DISABLED)
        self.btn_stop.pack(side=tk.LEFT, padx=10)
        self.btn_open_docs = tk.Button(btn_frame, text="打开 API 文档", command=self.open_docs, bg="#2196F3", fg="white", font=("Arial", 12, "bold"), padx=20, pady=10)
        self.btn_open_docs.pack(side=tk.LEFT, padx=10)

        self.service_process = None

        info_frame = tk.LabelFrame(frame, text="服务信息", padx=10, pady=10)
        info_frame.pack(fill=tk.X, pady=10)
        self.label_service_status = tk.Label(info_frame, text="状态: 未启动", fg="gray")
        self.label_service_status.pack()
        self.label_service_url = tk.Label(info_frame, text="访问地址: http://localhost:8000", fg="blue", cursor="hand2")
        self.label_service_url.pack()
        self.label_service_url.bind("<Button-1>", lambda e: self.open_docs())
    ...
    def start_service(self):
        ...
        venv_name = self.entry_service_venv_name.get()
        port = self.entry_service_port.get()
        host = self.entry_service_host.get()
        ...
        if not python_path.exists():
            messagebox.showerror("错误", f"找不到 Python: {python_path}\n请先安装依赖！")
            return

        env_file = project_root / ".env"
        if not env_file.exists():
            if not messagebox.askyesno("警告", "找不到 .env 文件，是否继续？"):
                return

        self.log("=" * 50)
        self.log("启动服务...")
        self.log(f"项目根目录: {project_root}")
        self.log(f"监听地址: {host}:{port}")

        try:
            cmd = [
                str(python_path), "-m", "uvicorn", "main:app",
                "--host", host, "--port", port, "--log-level", "info", "--access-log"
            ]
            ...
            self.label_service_status.config(text=f"状态: 运行中 (PID: {self.service_process.pid})", fg="green")
            self.label_service_url.config(text=f"访问地址: http://localhost:{port}")
            self.btn_start.config(state=tk.DISABLED, bg="#9E9E9E")
            self.btn_stop.config(state=tk.NORMAL)

            threading.Thread(target=self._monitor_service, daemon=True).start()

            self.log(f"服务已启动，PID: {self.service_process.pid}", "success")
            self.log(f"API 文档: http://localhost:{port}/docs", "info")
    ...
    def _monitor_service(self):
        ...
        if self.service_process:
            ...
            self.label_service_status.config(text="状态: 已停止", fg="red")
            self.btn_start.config(state=tk.NORMAL, bg="#4CAF50")
            self.btn_stop.config(state=tk.DISABLED)
            self.log("服务已停止", "warning")
