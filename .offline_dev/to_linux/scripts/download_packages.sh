...
# Python版本要求: 3.11 或 3.12 (必须与内网Linux目标机器一致)
#
# 使用方法:
#   # 默认使用 PyPI 官方源
...
download_packages() {
    ...
    # Linux 平台使用 manylinux2014_x86_64
    ...
    # 下载源码包（部分包可能没有Linux wheel）
    echo -e "\n${YELLOW}补充下载源码包（无Linux wheel的包）...${NC}"
    python3 -m pip download \
        --requirement "${REQUIREMENTS_FILE}" \
        --dest "${PACKAGES_DIR}" \
        --no-binary :all: \
...
generate_manifest() {
    ...
    echo "# 目标平台: Linux x86_64 (manylinux2014)" >> "${manifest_file}"
    echo "" >> "${manifest_file}"
...
package_output() {
    ...
    echo -e "${GREEN}========================================${NC}"
    echo ""
    echo "请将以下文件复制到内网 Linux 机器:"
    echo "  1. ${output_file}"
    echo "  2. ${PROJECT_ROOT}/requirements.txt"
    echo "  3. ${PROJECT_ROOT}/.offline_dev/to_linux/scripts/install_packages.sh"
    echo ""
    echo "在内网机器上解压后运行 install_packages.sh 即可安装"
}

# 主流程
main() {
    echo "========================================"
    echo "  MyAPS API 离线依赖包下载工具 (Linux)"
    echo "========================================"
    echo ""
