...
class DataCleaner:
    ...
    async def validate_from_config(self, table_key: str, data: Dict[str, Any], staging_id: int = None) -> List[Dict]:
        ...
        # 0. 执行前置钩子（pre_validate_hook，支持多个函数）
        pre_hooks = config.get("pre_validate_hook")
        if pre_hooks:
            hooks = pre_hooks if isinstance(pre_hooks, list) else [pre_hooks]
            for hook in hooks:
                hook_errors = await hook(self, data, staging_id)
        ...
        # 9. 执行后置钩子（post_validate_hook，支持多个函数）
        post_hooks = config.get("post_validate_hook")
        if post_hooks:
            hooks = post_hooks if isinstance(post_hooks, list) else [post_hooks]
            for hook in hooks:
                hook_errors = await hook(self, data, staging_id, errors)
                errors.extend(hook_errors)
...
class StagingProcessor:
    ...
    async def process_staging(self, table_name: str, batch_size: int = 100, use_transaction: bool = True, max_batches: int = 100) -> Dict[str, int]:
        ...
        # 执行表级别前置钩子（仅一次，支持多个函数）
        config = STAGING_TABLE_CONFIG.get(table_name, {})
        context = {}
        pre_batch_hooks = config.get("pre_batch_hook")
        if pre_batch_hooks:
            hooks = pre_batch_hooks if isinstance(pre_batch_hooks, list) else [pre_batch_hooks]
            for hook in hooks:
                hook_result = await hook(self, table_name, context)
                if hook_result:
        ...
        # 执行表级别后置钩子（仅一次，支持多个函数）
        post_batch_hooks = config.get("post_batch_hook")
        if post_batch_hooks:
            hooks = post_batch_hooks if isinstance(post_batch_hooks, list) else [post_batch_hooks]
            for hook in hooks:
                hook_result = await hook(self, table_name, context, stats)
                if hook_result:
                    context.update(hook_result)
            logger.debug(f"表级别后置钩子执行完成: {context}")
